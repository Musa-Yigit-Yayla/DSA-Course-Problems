...
public:
    ...
    int orangesRotting(vector<vector<int>>& grid) {
        ...
        for(int i = 0; i < this->adjList.size(); i++){
            ...
            if(currValue == 1){
                this->findTime(currIndex, -1, minTime, 0);
    ...
    void findTime(const int startVertex, int currVertex, int minTime[], int currTime){
        if(currVertex == -1){
            ...
            if(currVertex == 1){
                ...
                for(int i = 0; i < currAdj.size(); i++){
                    int currAdjValue = currAdj.at(0).at(1);
                    int currAdjIndex = currAdj.at(0).at(0);
                    if(currAdjValue == 1 && currAdjIndex != startVertex){ //second logic allows us to avoid
