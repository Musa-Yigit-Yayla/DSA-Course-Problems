...
public:
    string longestPalindrome(string s) {
        ...
        bool** arr = new bool[s.size()][s.size()]; //array contains rowIndex as the start index and columnIndex as the end index (inclusive)
        ...
        for(int i = 1; i < s.size(); i++){
            if(this->isPalindrome(s.substr(i - 1), i)){
                arr[i - 1][i] = true;
            }
        }
